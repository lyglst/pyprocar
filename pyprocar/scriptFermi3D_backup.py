...
def plot_line(x,y):
    ...
    mlab.plot3d([x[0],y[0]],[x[1],y[1]],[x[2],y[2]],color=(1,1,1),tube_radius=0.02)
...
def bring_pnts_to_BZ(recLat,kvector_cart,kvector_red,br_points):
    ...
    movements = np.array([recLat[0],recLat[1],recLat[2],-1*recLat[0],-1*recLat[1],-1*recLat[2],-1*recLat[0]-1*recLat[1],-1*recLat[1]-1*recLat[2],-1*recLat[0]-1*recLat[1],-1*recLat[0]-1*recLat[1]-1*recLat[2],recLat[0]+recLat[1],1*recLat[1]+1*recLat[2],1*recLat[0]+1*recLat[1],1*recLat[0]+1*recLat[1]+1*recLat[2]])
    for ik in range(len(kvector_cart)):
        ik_copy = kvector_cart[ik][:]
        idirection = 0
        was_outside = False
        while is_outside([br_points,ik_copy]) and idirection<14:
        ...
        kvector_red[ik] = np.dot(ik_copy,np.linalg.pinv(recLat)).round(3)
    if len(outsides):
        has_points_out = True
    else :  
...
def fft_interpolate(function,scale):
    eigen_fft = np.fft.fftn(function,norm="ortho")
    ...
    interpolated = np.abs(np.fft.ifftn(new_matrix,norm="ortho"))*(scale**3)
    return interpolated
...
#    has_points_out = False
    ...
    for iband in bands :
        ...
        mapped_func = np.pad(mapped_func[:-1,:-1,:-1],((padding_x,padding_x),(padding_y,padding_y),(padding_z,padding_z)),'wrap')
